...
function renderSourceTweet(post, target, logo) {
	...
	credibility="Disabled";//credibility.toFixed(2);

	var sentiment = "Disabled";//post.metadata.sentiment;
	...
	aNewItem.setAttribute("id", "threadSourcePost")
        aNewItem.innerHTML = "<span class='label label-default'>Source Post</span><div class='list-group'>"+
	"<a target='_blank' href='http://twitter.com/statuses/"+identifier+"' class='list-group-item active'><h5 class='list-group-item-heading'>"+post.text+"</h5>"+
	"<p>"+moment+"</p>"+
	"</a>"+
			...
			"<ul class='twPc-Arrange'>"+
				"<li class='twPc-ArrangeSizeFit'>"+
					"<a href=''>"+
						...
						"<span class='twPc-StatValue'><img src='"+logo+"' height='20' width='20' alt=''></span>"+
					"</a>"+
				"</li>"+
				"<li class='twPc-ArrangeSizeFit'>"+
					"<a href=''>"+
...
//render a source tweet stored as a SUPER post
function renderSourceFB(post, target, nopreview, logo) {

	var identifier = post.identifier;
	var timestamp = parseInt(post.timestamp)/1000;
	var moment = timeConverter(timestamp);
	var likes = 	post.metadata['likesCount'];

	var fromCategory = post.metadata['from.category'];

	var shares = post.metadata['shares.count'];

	var link = post.metadata['link'];
	...
	var type = post.metadata['type'];
	if (type === "photo") {
		mediaBlock = "<div class='thumbnail'><a href='"+link+"'><img src='"+post.metadata['picture']+"' alt='' onerror=\"this.src='"+nopreview+"'\"></a></div>";
	}
	...
	aNewItem.setAttribute("id", "threadSourcePost")
        aNewItem.innerHTML = "<span class='label label-default'>Source Post</span><div class='list-group'>"+
	"<a target='_blank' href='"+link+"' class='list-group-item active'><h5 class='list-group-item-heading'>"+post.text+"</h5>"+
	"<p>"+moment+"</p>"+
	"</a>"+
	"<div class='twPc-divStats'>"+
			"<ul class='twPc-Arrange'>"+
			...
			"</li>"+
				"<li class='twPc-ArrangeSizeFit'>"+
					"<a href=''>"+
						"<span class='twPc-StatLabel twPc-block'>Post Type</span>"+
						"<span class='twPc-StatValue'>"+type+"</span>"+
					"</a>"+
				"</li>"+
				"<li class='twPc-ArrangeSizeFit'>"+
					"<a href=''>"+
						"<span class='twPc-StatLabel twPc-block'>User Type</span>"+
						"<span class='twPc-StatValue'>"+fromCategory+"</span>"+
					"</a>"+
				"</li>"+
				"<li class='twPc-ArrangeSizeFit'>"+
				"<a href=''>"+
					"<span class='twPc-StatLabel twPc-block'>Shares</span>"+
					"<span class='twPc-StatValue'>"+shares+"</span>"+
				"</a>"+
				"</li>"+
				"<li class='twPc-ArrangeSizeFit'>"+
					"<a href=''>"+
						"<span class='twPc-StatLabel twPc-block'>Likes</span>"+
						"<span class='twPc-StatValue'>"+likes+"</span>"+
					"</a>"+
				"</li>"+
				"<li class='twPc-ArrangeSizeFit'>"+
					"<a href=''>"+
...
function renderThreadStats(thread, target) {
	...
	var positivePosts = "Disabled";//thread.statisicalData['positivePosts'];
	var negativePosts = "Disabled";//thread.statisicalData['negativePosts'];
...
function renderReplies(thread, target) {
	...
	for ( var i = 0; i < replies.length; i++ ) {
		...
		if (thread.sourcePost.type === "FACEBOOKPOSTS") {
			addTimelineItemFB(replyPost);
			addTimelineItemMedia(replyPost)
		} else {
			addTimelineItemTweet(replyPost);
			addTimelineItemMedia(replyPost)
		}
...
function addTimelineItemFB(post) {

	var identifier = post.identifier;
	var timestamp = parseInt(post.timestamp)/1000;
	var moment = timeConverter(timestamp);
	var likes = post.metadata['likeCount'];
	var text  = post.text;

	var comments = post.metadata['commentCount'];
	...
	var html = "<article id=\"post-"+identifier+"\" identifier=\""+identifier+"\" class='panel panel-primary'>"+
            "<div class='panel-heading icon'>"+
                "<i style='color: white' class='glyphicon glyphicon-comment'></i>"+
            "</div>"+
            "<div class='panel-body' identifier=\""+identifier+"\">"+
                text+
                "<div class='twPc-divStats'>"+
			"<ul class='twPc-Arrange'>"+
				"<li class='twPc-ArrangeSizeFit'>"+
					"<a href=''>"+
						"<span class='twPc-StatLabel twPc-block'>Likes</span>"+
						"<span class='twPc-StatValue' style='font-size: 14px'>"+likes+"</span>"+
					"</a>"+
				"</li>"+
				"<li class='twPc-ArrangeSizeFit'>"+
					"<a href=''>"+
						"<span class='twPc-StatLabel twPc-block'>Comments</span>"+
						"<span class='twPc-StatValue' style='font-size: 14px'>"+comments+"</span>"+
					"</a>"+
				"</li>"+
				"<li class='twPc-ArrangeSizeFit'>"+
					"<a href=''>"+
...
function addTimelineItemTweet(post) {

	var identifier = post.identifier;
	var text  = post.text;
	var retweets = 	post.metadata['retweet_count'];
	var lang = 	post.metadata['lang'];
	...
	var html = "<article id=\"post-"+identifier+"\" identifier=\""+identifier+"\" class='panel panel-primary'>"+
            "<div class='panel-heading icon'>"+
            	"<a target='_blank' href='http://twitter.com/statuses/"+identifier+"'>"+
                "<i style='color: white' class='glyphicon glyphicon-comment'></i>"+
                "</a>"+
            "</div>"+
            "<div class='panel-body' identifier=\""+identifier+"\">"+
